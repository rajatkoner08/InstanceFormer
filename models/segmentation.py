...
class SeqFormer(nn.Module):
    def __init__(self, detr, rel_coord=True, online=True, num_frames=1, freeze_detr=False):
        super().__init__()
        self.detr = detr
    ...
    def forward(self, samples: NestedTensor, gt_targets, criterion, train=False):    
        ...
        if self.online:
            ...
            for frame in range(self.num_frames):
                ...
                hs, hs_box, memory, init_reference, inter_references, inter_samples, enc_outputs_class, valid_ratios = self.detr.transformer(
                    frame_lbl_src, frame_lbl_mask, frame_lbl_pos, query_embed, tgt if hs==None else hs[-1,...].squeeze(1))
                hss.append(hs)
                memories.append(memory)
                init_ref.append(init_reference)
                inter_ref.append(inter_references)
                v_ratios.append(valid_ratios)

        else:
            hs, hs_box, memory, init_reference, inter_references, inter_samples, enc_outputs_class, valid_ratios = self.detr.transformer(srcs, masks, poses, query_embed, tgt)
        ...
        for lvl in range(enc_lay_num):
            ...
            indices = criterion.matcher(outputs_layer, gt_targets, self.num_frames, valid_ratios)
