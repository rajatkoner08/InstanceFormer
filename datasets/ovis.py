...
class OVISDataset:
    ...
    def __getitem__(self, idx):
        ...
        while not instance_check:
            ...
            target = self.ovis.loadAnns(ann_ids)
...
def make_coco_transforms(image_set, debug=False):
    normalize = T.Compose([
        T.ToTensor(),
...
def build(image_set, args):
    ...
    if 'ovis' in args.dataset_file:
        PATHS = {
            "train": (root / "train", root / "annotations_train.json"),
            "val": (root / "valid", root / "annotations_valid.json"),
            "test": (root / "test", root / "annotations_test.json"),
        }
